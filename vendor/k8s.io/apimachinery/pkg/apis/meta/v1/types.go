...
// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object
// 每一个资源都属于一个或多个资源版本，资源所属的版本如下结构描述，一个或多个资源版本通过Versions []string字符串数组进行存储。
...
// 没有组名的资源组: 被称为Core Group(即核心资源组)或Legecy Groups, 也可被称为GroupLess (即无组)。其表现形式为/<version>/<resource>,例如/v1/pods。
// 两类资源组表现形式不同，形成的HTTPPATH路径也不同。
// 拥有组名的资源组的HTTP PATH以/apis为前缀，其表现形式为/apis/<group>/<version>/<resource>，例如http://localhost:8080/apis/apps/v1/deployments
// 没有组名的资源组的HTTP PATH以/api 为前缀，其表现形式为/api/<version>/<resource>， 例如http://localhost:8080/api/v1/pods.
type APIGroup struct {
	...
	// +optional
	// 首选版本。
	// 当一个资源组内存在多个资源版本时，Kubernetes APl Server在使用资源时会选择一个首选版本作为当前版本。
...
type APIResource struct {
	...
	// 资源的单数名称，它必须由小写字母组成，默认使用资种类(Kind)的小写形式讲行命名。例如，Pod 资源的单数名称为pod，复数名称为pods。
...
// is namespaced.
// 描述所有Group、Versio、 Resource 的结构，每个资源都可以使用metav1.APIResourceList结构进行描述。
type APIResourceList struct {
	TypeMeta `json:",inline"`
	// groupVersion is the group and version this APIResourceList is for.
	// 描述资源组和资源版本，当资源同时存在资源组和资源版本时，它被设置为<group>/<version>
	// 当资源不存在资源组时，被设置为/<version>
	GroupVersion string `json:"groupVersion" protobuf:"bytes,1,opt,name=groupVersion"`
